...
def listar_produtos(*listas):
    """DEF recebe várias listas"""
    unique = {}
    lista_de_produtos = []
    for lista in listas:
        for prod, est in lista:
            lista_de_produtos.append(prod)
            unique = set(lista_de_produtos)
    print(f'{unique}')
    return 'Quantidade de Itens:', len(unique)
...
"""
x  = listar_produtos(estoque_fim_jan, estoque_fim_fev, estoque_fim_mar, estoque_fim_abr)
...
for I in nova_lista:
    ...
    qubra = str(I)
    print(qubra.replace(',', '\n'))
print(comp, 'Itens nas Listas')


# Replace ->  https://www.geeksforgeeks.org/python-string-replace/
"""
