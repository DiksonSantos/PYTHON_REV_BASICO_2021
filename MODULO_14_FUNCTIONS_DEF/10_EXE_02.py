...
"""
for i in vendas:
    # print(vendas[i][2], end='')
    if 'Cancelad' in vendas[i][1]:
    # print(vendas[i][2])
        print('Sure: ', vendas[i])
    if 'Cancelada pelo Cliente' in vendas[i][::]:
        print('sim')
# Chave [0] (16358, 'Cancelado', 'Estoque em Falta')
# Chave [1] Valor -> 16358
# Chave [2] Status -> 'Cancelado'
# Chave [3] Motivo (Quando ha cancelamento) -> blablabla
"""


def calc_stock(dic_vend):
    numerador = 0
    denominador = 0
    for venda in dic_vend:
        valor, status, motivo = dic_vend[venda]
        # Se a venda foi feita
        if status == 'Concluído':
            # ...o $ dela é adicionado a esta variavel
            denominador += valor
        elif status == 'Cancelado' and motivo == 'Estoque em Falta':  # or motivo == 'Cancelada pelo Cliente' :
            denominador += valor
            numerador += valor
    return numerador / denominador


# print(f"{calc_stock(vendas):.2%}")
...
# Identificada a posição de Quantidade de Dias que o CLiente esta devendo:
# for D in clientes_devedores:
#    print(D[2])
...
def inad(clientes):
    devedores_tot = 0
    CPFs = []
    print('Clientes inadimplentes a mais de 20 Dias:')
    for I in clientes:
        if I[1] > 1000 and I[2] >= 20:
            print(f'Saldo devedor -> {I[1]}R$\n'
                  f'Dias Inadimplente -> {I[2]} Dias\n'
                  f'CPF: {I[0]}'
                  )
            devedores_tot += 1
            CPFs.append(I[0])
    print(f'Quantidade total de Clientes Devedores = {devedores_tot}')
    for C in CPFs:
        print(f'CPFs Inadimplentes: {C}', end=' | ')
    print(f'\nTotal (Lista) de Inadimplentes -> {len(CPFs)}')


inad(clientes_devedores)


# OBS:  Returns com mais de uma resposta == Return arg1, arg2, arg3    -> Retorna uma tupla de respostas
...
quantidade = """CPF: 462.286.561-65
Saldo devedor -> 8177R$
Dias Inadimplente -> 28 Dias
CPF: 297.681.579-21
Saldo devedor -> 18826R$
Dias Inadimplente -> 29 Dias
CPF: 810.442.219-10
Saldo devedor -> 12445R$
Dias Inadimplente -> 24 Dias
CPF: 908.507.760-43
Saldo devedor -> 5364R$
Dias Inadimplente -> 22 Dias
CPF: 204.169.467-27
Saldo devedor -> 13932R$
Dias Inadimplente -> 20 Dias
CPF: 358.890.858-95
Saldo devedor -> 17048R$
Dias Inadimplente -> 25 Dias
CPF: 786.547.940-70
Saldo devedor -> 5806R$
Dias Inadimplente -> 21 Dias
CPF: 540.685.100-32
Saldo devedor -> 7622R$
Dias Inadimplente -> 24 Dias
CPF: 379.729.796-80
Saldo devedor -> 13167R$
Dias Inadimplente -> 24 Dias
CPF: 980.173.363-94
Saldo devedor -> 19581R$
Dias Inadimplente -> 24 Dias
CPF: 833.285.374-56
Saldo devedor -> 1545R$
Dias Inadimplente -> 30 Dias
CPF: 588.404.964-15
Saldo devedor -> 18079R$
Dias Inadimplente -> 28 Dias
CPF: 670.346.230-99
Saldo devedor -> 16581R$
Dias Inadimplente -> 23 Dias
CPF: 771.352.915-13
Saldo devedor -> 13942R$
Dias Inadimplente -> 24 Dias
CPF: 430.314.324-46
Saldo devedor -> 5888R$
Dias Inadimplente -> 30 Dias
CPF: 406.133.151-17
Saldo devedor -> 17316R$
Dias Inadimplente -> 30 Dias
CPF: 310.985.894-64
Saldo devedor -> 18818R$
Dias Inadimplente -> 30 Dias
CPF: 964.317.132-30
Saldo devedor -> 19369R$
Dias Inadimplente -> 29 Dias
CPF: 781.995.738-18
Saldo devedor -> 3206R$
Dias Inadimplente -> 27 Dias
CPF: 921.558.128-63
Saldo devedor -> 10228R$
Dias Inadimplente -> 26 Dias
CPF: 941.386.982-65
Saldo devedor -> 5212R$
Dias Inadimplente -> 20 Dias
CPF: 279.775.182-54
...
Dias Inadimplente -> 23 Dias
CPF: 165.554.107-13
Saldo devedor -> 10534R$
Dias Inadimplente -> 25 Dias
CPF: 718.225.984-87
Saldo devedor -> 13651R$
Dias Inadimplente -> 24 Dias
CPF: 397.994.286-79
Saldo devedor -> 8997R$
Dias Inadimplente -> 25 Dias
...
Dias Inadimplente -> 30 Dias
CPF: 152.817.649-24
Saldo devedor -> 15176R$
Dias Inadimplente -> 25 Dias
CPF: 645.308.846-62
Saldo devedor -> 17442R$
Dias Inadimplente -> 21 Dias
CPF: 201.476.809-95
Saldo devedor -> 18576R$
Dias Inadimplente -> 26 Dias
CPF: 486.290.887-24
Saldo devedor -> 8027R$
Dias Inadimplente -> 23 Dias
...
Dias Inadimplente -> 27 Dias
CPF: 817.170.984-26
Saldo devedor -> 13362R$
Dias Inadimplente -> 25 Dias
CPF: 591.397.550-29
Saldo devedor -> 11678R$
Dias Inadimplente -> 23 Dias
CPF: 615.629.238-82
Saldo devedor -> 3906R$
Dias Inadimplente -> 25 Dias
CPF: 240.427.458-70
Saldo devedor -> 12638R$
Dias Inadimplente -> 21 Dias
CPF: 700.720.266-23
Saldo devedor -> 2139R$
Dias Inadimplente -> 26 Dias
CPF: 261.974.594-90
Saldo devedor -> 10176R$
Dias Inadimplente -> 23 Dias
CPF: 680.569.318-52
Saldo devedor -> 16244R$
Dias Inadimplente -> 20 Dias
CPF: 200.761.898-70
Saldo devedor -> 16269R$
Dias Inadimplente -> 23 Dias
CPF: 556.156.341-36
Saldo devedor -> 2662R$
Dias Inadimplente -> 23 Dias
...
Dias Inadimplente -> 23 Dias
CPF: 536.714.951-95
Saldo devedor -> 17547R$
Dias Inadimplente -> 24 Dias
CPF: 520.645.562-80
Saldo devedor -> 17651R$
Dias Inadimplente -> 28 Dias
CPF: 913.525.896-32
Saldo devedor -> 4953R$
Dias Inadimplente -> 25 Dias
CPF: 760.248.897-67
Saldo devedor -> 17172R$
Dias Inadimplente -> 20 Dias
CPF: 920.890.990-46
Saldo devedor -> 17500R$
Dias Inadimplente -> 21 Dias
CPF: 805.469.913-50
Saldo devedor -> 8602R$
Dias Inadimplente -> 22 Dias
CPF: 847.150.802-96
Saldo devedor -> 11225R$
Dias Inadimplente -> 21 Dias
CPF: 539.300.108-41
Saldo devedor -> 1286R$
Dias Inadimplente -> 21 Dias
CPF: 549.151.467-76
Saldo devedor -> 18905R$
Dias Inadimplente -> 22 Dias
CPF: 738.451.908-29
Saldo devedor -> 17533R$
Dias Inadimplente -> 25 Dias
CPF: 987.288.834-69
Saldo devedor -> 1871R$
Dias Inadimplente -> 29 Dias
CPF: 169.415.202-43
Saldo devedor -> 18150R$
Dias Inadimplente -> 28 Dias
CPF: 757.885.355-97
Saldo devedor -> 9878R$
Dias Inadimplente -> 26 Dias
...
Dias Inadimplente -> 22 Dias
CPF: 535.354.954-30
Saldo devedor -> 13580R$
Dias Inadimplente -> 21 Dias
CPF: 209.759.133-88
Saldo devedor -> 2592R$
Dias Inadimplente -> 22 Dias
CPF: 321.123.241-10
Saldo devedor -> 11435R$
Dias Inadimplente -> 21 Dias
CPF: 407.342.963-78
Saldo devedor -> 1247R$
Dias Inadimplente -> 24 Dias
CPF: 770.920.161-42
Saldo devedor -> 19550R$
Dias Inadimplente -> 22 Dias
CPF: 815.943.237-83
Saldo devedor -> 9503R$
Dias Inadimplente -> 29 Dias
CPF: 379.995.400-37
Saldo devedor -> 12265R$
Dias Inadimplente -> 29 Dias
CPF: 495.461.913-57
Saldo devedor -> 5647R$
Dias Inadimplente -> 28 Dias
CPF: 408.987.269-72
Saldo devedor -> 10654R$
Dias Inadimplente -> 23 Dias
CPF: 761.137.848-34
Saldo devedor -> 1531R$
Dias Inadimplente -> 23 Dias
...
Dias Inadimplente -> 25 Dias
CPF: 986.516.478-33
Saldo devedor -> 14985R$
Dias Inadimplente -> 23 Dias
CPF: 327.988.845-70
Saldo devedor -> 15977R$
Dias Inadimplente -> 24 Dias
CPF: 887.826.412-21
Saldo devedor -> 9409R$
Dias Inadimplente -> 25 Dias
CPF: 122.975.174-32
Saldo devedor -> 11893R$
Dias Inadimplente -> 22 Dias
CPF: 208.788.890-61"""

print(quantidade.count('CPF'))
